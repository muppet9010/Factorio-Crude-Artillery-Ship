...
local function onEntityBuild(e)
    ...
    elseif entity.type == "cargo-wagon" or entity.type == "fluid-wagon" or entity.type == "locomotive" or entity.type == "artillery-wagon" then
        ...
        local engine = nil
        if entity.name == "cargo_ship" or entity.name == "oil_tanker" or entity.name == "artillery_ship" then
            local pos
            local dir
...
local function onTileBuild(e)
    if e.item and e.item.name == "landfill" then
        ...
        for _, tile in pairs(e.tiles) do
            if not surface.can_place_entity {name = "tile_test_item", position = tile.position} and surface.can_place_entity {name = "tile_player_test_item", position = tile.position} then
                -- refund
        ...
        if player.character then
            for dis = 1, 10 do
                local targets =
                    ...
                    name = {"indep-boat", "indep-boat-0", "boat_engine", "cargo_ship_engine", "boat", "cargo_ship", "oil_tanker", "artillery_ship"}
                }
                local done = false
...
local function OnDeleted(e)
    if (e.entity and e.entity.valid) then
        ...
        if entity.name == "cargo_ship" or entity.name == "oil_tanker" or entity.name == "boat" or entity.name == "artillery_ship" then
        ...
        elseif entity.name == "cargo_ship_engine" or entity.name == "boat_engine" then
            if entity.train then
                if entity.train.front_stock then
                    if entity.train.front_stock.name == "cargo_ship" or entity.train.front_stock.name == "oil_tanker" or entity.train.front_stock.name == "boat" or entity.train.front_stock.name == "artillery_ship" then
                        entity.train.front_stock.destroy()
                    end
                end
                if entity.train.back_stock then
                    if entity.train.back_stock.name == "cargo_ship" or entity.train.back_stock.name == "oil_tanker" or entity.train.back_stock.name == "boat" or entity.train.back_stock.name == "artillery_ship" then
                        entity.train.back_stock.destroy()
                    end
                end
            end
...
local function OnMined(e)
    if (e.entity and e.entity.valid) then
        ...
        if entity.name == "cargo_ship" or entity.name == "oil_tanker" or entity.name == "boat" or entity.name == "artillery_ship" then
            ...
            if entity.train then
                local engine
                if entity.train.back_stock and (entity.train.back_stock.name == "cargo_ship_engine" or entity.train.back_stock.name == "boat_engine") then
                    engine = entity.train.back_stock
                elseif entity.train.front_stock and (entity.train.front_stock.name == "cargo_ship_engine" or entity.train.front_stock.name == "boat_engine") then
...
local function init()
    ...
    global.last_cursor_stack_name = ((type(global.last_cursor_stack_name) == "table") and global.last_cursor_stack_name) or {}
    global.last_distance_bonus = ((type(global.last_distance_bonus) == "number") and global.last_distance_bonus) or settings.global["waterway_reach_increase"].value
    global.current_distance_bonus = settings.global["waterway_reach_increase"].value
...
local deleted_filters = {
    ...
    {filter = "name", name = "artillery_ship"},
